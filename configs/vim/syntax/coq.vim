...
" Last Change: 2015 Jul 08 - Update for some new keywords and fix a few bugs with newer syntax
"              2008 Dec 02 - Added the Program and Obligation constructions (in Coq v8.2) - with Serge Leblanc.
...
" Various
syn region coqRequire contains=coqString matchgroup=coqVernacCmd start="\<From\>\%(\_s\+\%(Export\|Import\)\>\)\=" matchgroup=coqVernacPunctuation end="\.\_s"
...
syn region coqSection contains=coqSection,coqModule,@coqVernac matchgroup=coqVernacCmd start="\<Section\_s*\z(\S\+\)\_s*\.\_s" end="\<End\_s\+\z1\_s*\.\_s"
syn region coqModule contains=coqSection,coqModule,@coqVernac matchgroup=coqVernacCmd start="\<Module\%(\_s\+\%(Type\|Import\|Export\)\)\=\_s*\z(\S\+\)\%(\_s*:\%(\_s*\i\)\{-}\)\=\%(\_s*<:\%(\_s*\S\)\{-}\)\=\_s*\.\_s" end="\<End\_s\+\z1\_s*\.\_s"
...
syn region coqObligation contains=coqOblExpr matchgroup=coqVernacCmd start="\<\%(Local\_s\+\)\=Obligations\=\_s\+Tactic\>" end="\.\_s" keepend
...
syn region coqOpen    contains=coqScope matchgroup=coqVernacCmd start="\<\%(Local\_s\+\)\=Open\>" matchgroup=coqVernacPunctuation end="\.\_s" keepend
...
syn region coqHintOption start="\<\%(Resolve\|Rewrite\|Immediate\|Constructors\|Unfold\|Extern\)\>" end="\.\_s"
...
" Notations
syn region coqNotation     contains=coqNotationDef start="\%(\%(\%(\<Local\>\_s*\)\=\%(\<Reserved\>\_s*\)\?\<Notation\>\)\|\<Infix\>\)" matchgroup=coqVernacPunctuation end="\.\_s" keepend
...
" Declarations 
syn region coqDecl       contains=coqIdent,coqDeclTerm,coqDeclBinder matchgroup=coqVernacCmd start="\<\%(Axiom\|Conjecture\|Hypothes[ie]s\|Parameters\=\|\%(Generalizable\_s\+\)\=Variables\?\)\>" matchgroup=coqVernacCmd end="\.\_s" keepend
...
syn keyword coqTactic    contained absurd apply assert assumption auto tauto admit
...
syn keyword coqTactic    contained set cbn simpl[e] simplify_eq split subst stepl stepr symmetry
...
" Definitions
syn region coqDef          contains=coqDefName matchgroup=coqVernacCmd start="\<\%(Program\_s\+\)\=\%(Definition\|Instance\|Let\)\>" matchgroup=coqVernacPunctuation end=":="me=e-2 end="\.$"me=e-1 end="\.\s"me=e-2 nextgroup=coqDefContents1,coqProofBody keepend skipnl skipwhite skipempty
...
" Classes
syn region coqClass        contains=coqClassProfile start="\<Class\>" matchgroup=coqVernacPunctuation end="\.\_s" keepend
syn region coqClassProfile contained contains=coqIdent,coqClassTerm,coqClassBinder matchgroup=coqVernacCmd start="Class" matchgroup=NONE end="\.\_s"
syn region coqClassBinder  contained contains=@coqTerm matchgroup=coqTermPunctuation start="(" end=")"
syn region coqClassTerm    contained contains=@coqTerm,corClassContent matchgroup=coqVernacPunctuation start=":=" end="\.\_s"
syn region coqClassContent contained contains=coqConstructor,coqClassStart matchgroup=coqVernacPunctuation start=":=" end="\.\_s"
syn region coqClassStart   contained contains=coqClassField,@coqTerm start="{" matchgroup=coqVernacPunctuation end="}" keepend
syn region coqClassField   contained contains=coqField matchgroup=coqVernacPunctuation start="{" end=":"
syn region coqClassField   contained contains=coqField matchgroup=coqVernacPunctuation start=";" end=":"
